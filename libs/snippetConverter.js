...
function processSnippetFolder(folderPath, outputFileName) {
  ...
  // jsonOutput.replace(/\\\\\$/g, '\$')
  // 新版本的vscode不能将 \\$ 替换为 $ 否则在插入vscode的时候$var 就变成了 var
  fs.writeFile(outputFileName, jsonOutput, function (err) {
    if (err) throw err;
    console.log("File wrote to " + outputFileName);
  });
  console.log(
    count +
      " snippet(s) found and converted." +
      (errors.length > 0
    ...
    files.forEach(function (fileName) {
      ...
      if (snippet) {
        if (snippet.prefix && snippet.body) {
          ...
          if (!snippet.prefix) {
            errors.push(
              filePath + ": Missing property 'tabTrigger'. Snippet skipped."
            );
          } else {
            errors.push(
              filePath + ": Missing property 'content'. Snippet skipped."
            );
          }
        }
      }
  ...
  function convertSublime(filePath) {
    ...
    parser.onerror = function (e) {
      errors.push(
        filePath + ": Problems parsing content content: " + e.message
      );
    };
